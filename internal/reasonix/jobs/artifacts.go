...
import (
	...
	fileencoding "scopeforge/internal/reasonix/fileutil/encoding"
)

// sessionJobsDir 是会话 job 伴生目录(替代上游 store.SessionJobsDir:
// <sessionPath 的父目录>/jobs/<session 名>)。本平台 M0 会话持久化走 SQLite,
// 后台任务侧车文件仅用于 job 日志/元数据,按会话 id 落盘。
func sessionJobsDir(sessionPath string) string {
	...
	return filepath.Join(dir, "jobs", base)
}
...
func ArtifactDir(sessionPath string) string {
	return sessionJobsDir(sessionPath)
}
