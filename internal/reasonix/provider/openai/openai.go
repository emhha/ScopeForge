...
import (
	...
	"scopeforge/internal/reasonix/netclient"
...
func (c *client) readStream(ctx context.Context, resp *http.Response, out chan<- provider.Chunk) (emitted bool, _ error) {
	...
	var usage *provider.Usage // 缓存流末 usage(SiliconFlow 逐 chunk 附 usage,统一发一次)
	...
	for scanner.Scan() {
		...
		if sr.Usage != nil {
			// 部分 OpenAI 兼容网关(如 SiliconFlow)在每个 SSE chunk 都附 usage
			// 累计值,标准 OpenAI/DeepSeek 只在流末尾发一次。若逐 chunk 转发,
			// 消费方(executor)会按 chunk 记账,成本与 turn 计数虚高百倍。
			// 修复:缓存,流正常结束时统一发送一次。
	...
	// 流正常结束时统一发送一次 usage(缓存值;中断的流不记账)。
	if usage != nil {
		emitted = true
